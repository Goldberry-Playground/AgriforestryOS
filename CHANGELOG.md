...
- CI: scheduled missing-required-check sweep (`missing-checks-sweep.yml` on `schedule` + `workflow_dispatch`, GOL-1958) — detects a required status check that never *starts* (a dropped `pull_request` trigger) and would otherwise hang a PR indefinitely at "Expected — waiting for status"; routes the finding through the shared `ci-failure-router` as a deduped alert with the exact close/reopen remediation.
...
- AgriforestryOS fork: prod tier + required-reviewer approval gate (Sprint 7, production-readiness blocker #4) — the "human approves → prod" step. New `deploy-prod.yml` (promote a `v*` tag / dispatched ref) and `migrate-prod.yml` (manual `drush updb`, snapshot-first) both run under the **`prod` GitHub Environment**, which pauses each run for a **required reviewer** before anything touches production. Prod is a separate, isolated tier: a Terraform **workspace** (`-var environment=prod`, separate state) with a **block-storage volume** (`backups_volume_size_gb`) mounted at `/opt/agriforestryos/backups` so local backups survive droplet replacement; the compose `backups` volume now binds to that path (works on dev disk too). Secrets are **environment-scoped** (`DEPLOY_PROD_HOST` / `DEPLOY_PROD_SSH_KEY` / `PROD_ENV_FILE`), armed by `DEPLOY_PROD_ENABLED`. The prod deploy validates its ref against shell-metacharacter injection before use. Full runbook: `docs/hosting/prod-deploy.md` (incl. the `gh api` to create the Environment + reviewer). dev unchanged.
- AgriforestryOS fork: private-mesh hardening (Sprint 7, production-readiness blocker #3) — the droplet joins a **Tailscale** tailnet on boot (cloud-init) and the DigitalOcean **cloud firewall** drops public `80`/`443`, leaving only `22/tcp` (SSH break-glass + CI deploy) and `41641/udp` (Tailscale). farmOS is now reachable **only over the tailnet**, where WireGuard encrypts all transit — so basic auth over http is acceptable and no public TLS certs / OAuth2 migration are needed. (The cloud firewall is the real control: Docker's published ports bypass `ufw`, but not the NIC-level cloud firewall.) New required Terraform input `tailscale_auth_key` (sensitive; via `TF_VAR_…`), a `farmos_url_note` output, and updated `infra/terraform/` README + `docs/hosting/dev-deploy.md` (operator access is now tailnet-based; the MCP server points `FARMOS_BASE_URL` at the MagicDNS name). Local dev and the CI deploy path (public `:22`) are unchanged.
- AgriforestryOS fork: production farmOS image (Sprint 7, production-readiness blocker #2) — `docker/farmos.Dockerfile` pins the farmOS base **by digest** (resolved from `farmos/farmos:4.x-dev`, not the moving tag) and **bakes the `farm_syntropic` module in** (no live bind-mount, `XDEBUG_MODE=off`). `docker-compose.prod.yml`'s `www` now builds this image (context = repo root) instead of mounting the dev image + module; `up -d --build` (what the deploy workflow runs) ships committed module changes, and the pinned digest is the one place farmOS advances in prod — bumped deliberately. Local dev is unchanged (`docker-compose.development.yml` keeps the bind-mount). New advisory `farmos-image-ci.yml` builds the image on Dockerfile/module changes to guard the pinned digest + COPY. (The deployed stack is now reproducible and self-contained — no moving tag, no bind-mount.)
- AgriforestryOS fork: `backup-service/` — database backup/restore (Sprint 7, production-readiness blocker #1). The farmOS DB is the irreplaceable system of record and had no backups; this adds daily `pg_dump -Fc` of both the farmOS DB and the PostGIS mirror, written **both** to a local volume and to **DigitalOcean Spaces**, with independent retention pruning (`BACKUP_KEEP` newest per database in each place; `0` = never prune). Degrades to **local-only with a warning** if `SPACES_*` creds are absent — never silently skips a backup; one DB's `pg_dump` failure never aborts the other. `restore.py` does manual `pg_restore --clean` from a local dump or `--from-s3`. Pure core (`dump_filename`, `prune_plan`, `s3_key`) + `BackupService` orchestration with `pg_dump`/filesystem/boto3 faked — 12 tests, no live Postgres/Spaces needed. Wired as the `backup` compose service (+ `backups` volume) with a per-package CI workflow; the manual **Run DB migrations** workflow now takes a `--reason premigrate` snapshot **before** `drush updb` (the rollback point for a bad update hook). Docs in `docs/hosting/backups.md`.
- AgriforestryOS fork: harvest → Odoo receipt sync (Sprint 6) — `sync-service/harvest_sync.py`, the reverse (farmOS → Odoo, event-driven) of the nursery sync. Reads new harvest logs, maps each harvested quantity to an Odoo **production `stock.move`** (virtual Production → stock, increasing on-hand qty of the matched product), stamping the farmOS log UUID as the move `origin` — which is also the idempotency key (Odoo is the dedup source of truth; re-runs never double-record). Product matched best-effort by name (quantity label → product); unmatched are skipped + logged, not guess-created. farmOS fraction-quantities normalized to numbers. Runs as a second service off the same sync-service image (`harvest-sync` in `compose.prod`, separate cursor + poll). 9 tests (pure flatten/map + orchestration with fakes). **Odoo modeling (production stock.move) is the reviewable assumption.**
- AgriforestryOS fork: dev deploy pipeline — `docker/docker-compose.prod.yml` (one server stack: farmOS + Postgres + PostGIS + sync-service + postgis-etl; config from `.env`, DB ports unpublished), `.github/workflows/deploy-dev.yml` (auto app-deploy on merge to `4.x` — SSH to the droplet, pull, `compose up --build`, `drush cr`; **never** runs migrations), and `.github/workflows/migrate-dev.yml` (manual `workflow_dispatch` for `drush updb` — the deliberate, human-triggered migration step). Both workflows are dormant until `vars.DEPLOY_ENABLED == 'true'`. Secrets flow 1Password → GitHub secrets → server `.env` at deploy time (never committed). Runbook in `docs/hosting/dev-deploy.md`. MCP server + QGIS stay operator-local (not deployed).
- AgriforestryOS fork: `infra/terraform/` — Terraform IaC for the **dev environment** on DigitalOcean. Provisions a Docker-host droplet (Ubuntu 24.04, 4GB default) with cloud-init (installs Docker, creates a `deploy` user, clones the repo, sets `ufw`), a **reserved IP** (stable deploy target across rebuilds), and a **firewall** that exposes only 22/80/443 — Postgres/PostGIS stay off the public internet. API token via `DIGITALOCEAN_TOKEN` env (never in code); state gitignored. This is the infrastructure half of the planned `code → CI → auto-deploy → dev` pipeline; GitHub Actions will deploy the app onto `terraform output reserved_ip`. `terraform validate` clean.
- AgriforestryOS fork: `list_harvests` + `harvest_summary` MCP tools (Sprint 6) — read the farmOS harvest log for yield tracking. `list_harvests` returns harvest records with harvested assets and quantities (value / units / measure / label, with farmOS fraction values normalized to numbers), filterable by asset and date range; `harvest_summary` aggregates quantities by `asset`, `month`, or `measure`. Pure flatten/aggregate logic in `harvest.py`, 10 tests over a representative JSON:API fixture — no live farmOS needed. Foundation for the Odoo harvest→receipt sync. MCP server now exposes 8 tools.
- AgriforestryOS fork: `spatial_query` MCP tool (Sprint 5) — sixth tool on the farmOS MCP server, running proximity queries against the PostGIS mirror. Finds assets of a type (`trees` / `infrastructure` / `plantings` / `land_areas`) near a WGS84 point, ordered nearest-first with a `distance_m` field; optional `within_m` radius (ST_DWithin, GiST-index-assisted) answers "trees within 30 m of the well", omitting it gives nearest-N. SQL builder is pure/allow-listed (no injection surface — table/columns never interpolated, all values bound); execution via psycopg. PostGIS is optional — the tool returns a clear error if `POSTGIS_DSN` is unset and the other five tools are unaffected. Ships `spatial.py` + 8 tests (pure builder + fake-psycopg runner).
- AgriforestryOS fork: `postgis-etl/` — farmOS → PostGIS spatial-mirror ETL (Sprint 5). Mirrors Tree / Infrastructure / Tree Planting / Land asset geometries into PostGIS so QGIS can draw live layers and the farmOS MCP server can run spatial SQL. Each pass fetches assets via JSON:API, flattens to rows (attributes + relationship names + `intrinsic_geometry` WKT), upserts keyed on asset UUID (reprojecting WGS84 → EPSG:26917 in-DB via `ST_Transform`), and prunes rows whose assets no longer exist upstream — idempotent, converges the mirror to match farmOS. Python 3.11+, uv, httpx + psycopg. Ships `schema.sql` (4 GiST-indexed tables), `farmos.py`, `extract.py`, `db.py`, `etl.py`, a `Dockerfile`, and 12 faked tests (no live farmOS/PostGIS needed).
- AgriforestryOS fork: `docker/docker-compose.postgis.yml` — compose override adding a `postgis/postgis:16-3.4` service (the spatial mirror, separate from farmOS's operational DB) on host port 5433, db `agriforestryos`. Schema auto-created by the ETL on first run.
- AgriforestryOS fork: `.github/workflows/postgis-etl-ci.yml` — advisory CI running the postgis-etl pytest suite on `postgis-etl/**` changes. Mirrors the other package CIs; pinned to `astral-sh/setup-uv@08807647` (v8.1.0).
...
- AgriforestryOS fork: `.github/workflows/sync-service-ci.yml` — advisory CI workflow running the sync-service pytest suite on every `sync-service/**` change. Mirrors `mcp-server-ci.yml`; pinned to `astral-sh/setup-uv@08807647` (v8.1.0).
...
- AgriforestryOS fork: `farm_syntropic_update_8002` — installs field storage for `tenure` and `parent_planting` on **existing** sites. Both fields were added to the Tree asset type in code (tree-planting + Odoo-sync work) without an update hook, so fresh installs worked (and CI's fresh-stack smoke test passed) but existing installs broke on every Tree read — UI, JSON:API, and the PostGIS ETL — with `Undefined table: asset__tenure` / `asset__parent_planting`. Surfaced by bringing up the dev stack against the existing Sprint-3.5 database. The hook guards on table existence, so it's a no-op on fresh installs and re-runs; validated against live Drupal 11.
- AgriforestryOS fork: `postgis-etl` now skips an asset type that returns 404 (not installed on this farmOS, e.g. `land`) instead of aborting the whole mirror pass. `FarmOSReadError` carries a `status_code`; the ETL treats 404 as "absent, skip" and re-raises everything else. +2 tests.
